...
<!--
Skeleton for a RomM release note, stable or alpha.1 / beta.1 prerelease. Drop any
section that has no content. Inline HTML and bare URLs are deliberate here:
GitHub renders the notes, and the bullet lines are copied verbatim out of the
autogenerated body.

The API changes table belongs to the prerelease that opens the line. A stable
release drops it, unless the line shipped no prerelease carrying one.

A release under roughly eight PRs with no feat: work, and every prerelease after
the first on its line, replaces everything from the callouts down to Other
changes with a single flat `## What's Changed` list carrying every autogenerated
bullet, then keeps New Contributors and the changelog link.
-->
