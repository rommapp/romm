...
- **Type-check with mypy**, outside Trunk so it sees the project's packages: `uv run mypy --config-file ../.trunk/configs/mypy.ini .` from `backend/`. CI's `mypy.yml` requires zero errors. Every function outside `tests/` needs full parameter and return annotations, and every `# type: ignore` names its error code.
