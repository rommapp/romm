...
- **Engine-specific query SQL:** reach for a portable SQLAlchemy expression first; if the engines need different SQL, add a `@compiles` construct to `utils/sql_dialect.py` rather than branching on `ROMM_DB_DRIVER` in a handler. Register MySQL-family compilers with `_compiles_on_mysql_family` (`"mysql"` alone misses MariaDB), and pin each spelling in tests by compiling for `MARIADB_DIALECT`/`POSTGRESQL_DIALECT` from `tests/handler/database/conftest.py`.
