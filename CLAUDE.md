...
**No em-dashes.** Never use em-dashes (—) when writing comments or text. Use commas, parentheses, or separate sentences instead. ESLint (`romm/no-emdash-in-comment`) enforces this for comments under `frontend/src/v2`; a `"—"` string is code (an empty-value glyph) and stays allowed.
**Write the check, don't delegate it to a model.** When a rule is mechanical (an import, a character, a CSS pattern, an SFC shape), it belongs in ESLint, `tsconfig`, or a CI script, not in a skill an LLM reviewer re-reads on every PR. Agents own this: when you hit or fix a repeatable review finding, turn on a stock rule or add one to `frontend/eslint-plugin-romm/` with a `RuleTester` test, in the same PR or as a named follow-up. Encode the rule that finds violations; fixing existing violations is separate work. Keep model review for judgment calls.
