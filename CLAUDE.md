...
**No em-dashes.** Never use em-dashes (U+2014) when writing comments or text. Use commas, parentheses, or separate sentences instead. ESLint (`romm/no-emdash-in-comment`) checks comments under `frontend/src/v2`, and Trunk's `no-emdash` checks Markdown and Python comments. An em-dash string literal is code (an empty-value glyph) and stays allowed.
**Prefer a check to a model review.** A mechanical rule (an import, a character, a CSS pattern, an SFC shape) belongs in ESLint, `tsconfig`, or Trunk. When a review finding repeats, propose the stock or `frontend/eslint-plugin-romm/` rule as a named follow-up rather than growing the current PR.
