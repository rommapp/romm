...
The service catalog comes from Apprise's own description of each plugin: its URL tokens become the main fields (a field is required only when every URL template needs it, since alternative credentials are each "required" in Apprise's eyes), and its options become advanced fields. A token is a secret when Apprise says so or its name says it is a credential (`token`, `key`, `secret`, `webhook`…). A channel stores the service and the fields it takes, sealed, and the backend builds the Apprise URL from the template those fields fill completely, at each delivery. Schemas that act on the host (`syslog`, `dbus`, `windows` and the like) and FCM (which opens its key file itself) aren't offered, nor are the options Apprise reads from a local file (`template`, `keyfile`, `subfile`, `pgppub`, `pgpkey`, `pgpprv`) or the ones RomM sets itself (timeouts, retries, redirects). Apprise follows no redirects, doesn't retry on its own, its timeouts are capped at 10 s to connect and 15 s to read, a list takes at most 20 items, and `@everyone`, `@here` and `<@…>` are broken up so they ping nobody. The API returns a channel's service, the fields that aren't secret and the names of the secrets it has, and its URL through Apprise's masking with any remaining secret hidden and no query. On edit, a secret left out keeps its value and an empty one removes it; a kept secret doesn't follow the channel to another host, port, schema, SMTP server or ntfy mode. A test gets 60 s in all, like a queued delivery. Only an admin's webhooks may reach private addresses; everyone else's go through the SSRF guard. A kept webhook secret doesn't follow the channel to another origin; it has to be given again. A webhook delivery gets 15 s in all, and only the start of a refusal's body is read. Text for RomM's own kinds is English until outbound messages are translated. Links are absolute only when `ROMM_BASE_URL` is shareable.
