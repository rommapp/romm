...
def is_bot_challenge(response: httpx.Response) -> bool:
    ...
    identify themselves in `Server` alongside the refusal status.
    """
...
async def fetch_gamefaqs_guide(url: str) -> ParsedGuide:
    ...
    try:
        # Streamed so the size cap bounds what is read, rather than truncating a
        # body already buffered in full.
    ...
    try:
        # A challenge page carries a 403 and challenge HTML, so it would
        # otherwise surface as an opaque status error or an empty parse.
        if is_bot_challenge(response):
            raise GameFAQsFetchError(
                "GameFAQs is blocking automated requests for this guide. Open "
                "the guide in your browser, save it as a text file, and upload "
                "it here instead."
            )
    ...
    content = b"".join(chunks)[:MAX_GUIDE_BYTES]
