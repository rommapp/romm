...
def enqueue_channel_deliveries(
    ...
    """
    try:
        channels = db_notification_channel_handler.get_deliverable_channels(
            sorted({user_id for user_id, _ in stored})
        )
        for user_id, notification in stored:
            payload = notification.model_dump(mode="json")
            for channel in channels:
                if channel.user_id == user_id and forwards(channel, notification):
...
def deliver_to_channel(channel_id: int, notification: dict[str, Any]) -> None:
    ...
    The last failed attempt returns rather than raises, so no failed job is
    left behind; the channel keeps the error and counts it instead.
    """
    found = db_notification_channel_handler.get_channel_for_delivery(channel_id)
    if found is None:
        return
    channel, owner_is_admin = found
    if not channel.enabled:
        return

    message = render(NotificationSchema.model_validate(notification))
    try:
        asyncio.run(send_to_channel(channel, message, allow_private=owner_is_admin))
