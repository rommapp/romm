...
class TGDBHandler(MetadataHandler):
    ...
    #     MANUFACTURER_REGEX = re.compile(r"Manufacturer: (.*)\<")
    #     DEVELOPER_REGEX = re.compile(r"Developer: (.*)\<")
    #     MEDIA_MEDIUM_REGEX = re.compile(r"Media Medium: (.*)\<")
    #     CPU_REGEX = re.compile(r"CPU: (.*)\<")
    #     MEMORY_REGEX = re.compile(r"Memory: (.*)\<")
    #     GRAPHICS_REGEX = re.compile(r"Graphics: (.*)\<")
    #     SOUND_REGEX = re.compile(r"Sound: (.*)\<")
    #     DISPLAY_REGEX = re.compile(r"Display: (.*)\<")
    #     MAX_CONTROLLERS_REGEX = re.compile(r"Max Controllers: (.*)\<")
    #     SUMMARY_REGEX = re.compile(r"card-body\">[^<]*<p>([^<]*)")

    #     for platform_key in TGDB_PLATFORM_LIST.keys():
    #         platform = TGDB_PLATFORM_LIST[platform_key]
    #         platform_id = platform["id"]

    #         url = f"https://thegamesdb.net/platform.php?id={platform_id}"
    #         res = await httpx_client.post(
    #             url,
    #             timeout=10,
    #         )

    #         # Get manufacturer from the HTML response
    #         if res.status_code != 200:
    #             continue

    #         html_content = res.text
    #         match = MANUFACTURER_REGEX.search(html_content)
    #         platform["manufacturer"] = match.group(1).strip() if match else None

    #         match = DEVELOPER_REGEX.search(html_content)
    #         platform["developer"] = match.group(1).strip() if match else None

    #         match = MEDIA_MEDIUM_REGEX.search(html_content)
    #         platform["media_medium"] = match.group(1).strip() if match else None

    #         match = CPU_REGEX.search(html_content)
    #         platform["cpu"] = match.group(1).strip() if match else None

    #         match = MEMORY_REGEX.search(html_content)
    #         platform["memory"] = match.group(1).strip() if match else None

    #         match = GRAPHICS_REGEX.search(html_content)
    #         platform["graphics"] = match.group(1).strip() if match else None

    #         match = SOUND_REGEX.search(html_content)
    #         platform["sound"] = match.group(1).strip() if match else None

    #         match = DISPLAY_REGEX.search(html_content)
    #         platform["display"] = match.group(1).strip() if match else None

    #         match = MAX_CONTROLLERS_REGEX.search(html_content)
    #         platform["max_controllers"] = match.group(1).strip() if match else None

    #         match = SUMMARY_REGEX.search(html_content)
    #         platform["summary"] = match.group(1).replace("\n", "").replace("\r", "").strip() if match else None

    #         platform["url_logo"] = None
    #         for i in range(1, 4):
    #             image_url = f"https://cdn.thegamesdb.net/images/original/platform/boxart/{platform_id}-{i}.jpg"

    #             # Fetch image to ensure it exists
    #             image_res = await httpx_client.get(image_url, timeout=10)
    #             if image_res.status_code == 200:
    #                 platform["url_logo"] = image_url
    #                 break

    #         new_platform_list[platform_key] = platform
    #         print(platform_key)
    #         print(platform)
