...
async def add_rom_gamefaqs_walkthrough(
    ...
    stat = os.stat(file_location)
    # The guide is on disk but nothing references it yet, so drop it again if
    # indexing fails rather than leaving an unreachable file behind.
    try:
        created = db_rom_handler.add_rom_file(
            RomFile(
                rom_id=rom.id,
        ...
        log.error("Error indexing GameFAQs walkthrough", exc_info=exc)
        with contextlib.suppress(OSError):
            await fs_rom_handler.remove_file(f"{walkthrough_dir_rel}/{file_name}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="There was an error saving the walkthrough",
        ) from exc
