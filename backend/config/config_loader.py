...
class ConfigLoader:
    def __init__(self):
        try:
            with open(ROMM_USER_CONFIG_PATH) as config_file:
    ...
    def _parse_config(self) -> dict:
        try:
            self.config["EXCLUDED_PLATFORMS"] = (
                ...
                if self.config["exclude"]["platforms"]
                else []
            )
        ...
        try:
            self.config["EXCLUDED_SINGLE_EXT"] = (
                ...
                if self.config["exclude"]["roms"]["single_file"]["extensions"]
                else []
            )
        except (KeyError, TypeError):
            self.config["EXCLUDED_SINGLE_EXT"] = []
        try:
            self.config["EXCLUDED_SINGLE_FILES"] = (
                ...
                if self.config["exclude"]["roms"]["single_file"]["names"]
                else []
            )
        except (KeyError, TypeError):
            self.config["EXCLUDED_SINGLE_FILES"] = []
        try:
            self.config["EXCLUDED_MULTI_FILES"] = (
                ...
                if self.config["exclude"]["roms"]["multi_file"]["names"]
                else []
            )
        except (KeyError, TypeError):
            self.config["EXCLUDED_MULTI_FILES"] = []
        try:
            self.config["EXCLUDED_MULTI_PARTS_EXT"] = (
                ...
                if self.config["exclude"]["roms"]["multi_file"]["parts"]["extensions"]
                else []
            )
        except (KeyError, TypeError):
            self.config["EXCLUDED_MULTI_PARTS_EXT"] = []
        try:
            self.config["EXCLUDED_MULTI_PARTS_FILES"] = (
                ...
                if self.config["exclude"]["roms"]["multi_file"]["parts"]["names"]
                else []
            )
        except (KeyError, TypeError):
            self.config["EXCLUDED_MULTI_PARTS_FILES"] = []
        try:
            self.config["PLATFORMS_BINDING"] = (
                ...
                if self.config["system"]["platforms"]
                else {}
            )
        except (KeyError, TypeError):
            self.config["PLATFORMS_BINDING"] = {}
