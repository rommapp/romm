...
# Alembic's own bookkeeping table; it has no model and never will.
UNMODELLED_TABLES = frozenset({"alembic_version"})


def _load_every_model() -> set[str]:
    """Populate `BaseModel.metadata` from every module under `models/`.

    A table absent from the metadata makes `compare_metadata` report each of
    its indexes as removed, so the comparison is only meaningful once all of
    them are registered. Walking the package rather than listing the modules
    keeps a newly added model covered without anyone remembering to.
    """
    for module in pkgutil.iter_modules(models.__path__):
        importlib.import_module(f"models.{module.name}")
    return set(BaseModel.metadata.tables)
...
def test_no_index_drift_between_models_and_migrations():
    ...
    """
    model_tables = _load_every_model()
    with sync_engine.connect() as connection:
        # Guard the comparison itself: an empty or partial metadata would make
        # the assertion below pass or fail for reasons unrelated to drift.
        unmodelled = (
        ...
        assert unmodelled == set(), (
            "tables exist in the database but not in the model metadata, so the "
            f"drift comparison would be unsound: {sorted(unmodelled)}"
        )
