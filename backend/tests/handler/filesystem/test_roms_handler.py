...
class TestFSRomsHandler:
    ...
    @pytest.mark.asyncio
    async def test_get_rom_files_audio_hashed_when_hashing_disabled(
        self, handler: FSRomsHandler, rom_multi, config
    ):
        """Soundtrack audio files get individual hashes even with hashing off;
        other files and the rom-level hash stay unhashed."""
        import hashlib

        audio_bytes = b"fake-mp3-bytes"
        soundtrack_dir = (
            handler.base_path / "n64/roms/Super Mario 64 (J) (Rev A)/soundtrack"
        )
        soundtrack_dir.mkdir(parents=True, exist_ok=True)
        (soundtrack_dir / "theme.mp3").write_bytes(audio_bytes)
        ...
        try:
            with pytest.MonkeyPatch.context() as m:
                ...
                parsed = await handler.get_rom_files(rom_multi, calculate_hashes=False)

            by_name = {rf.file_name: rf for rf in parsed.rom_files}
            audio = by_name["theme.mp3"]
            assert audio.md5_hash == hashlib.md5(audio_bytes).hexdigest()
            assert audio.sha1_hash == hashlib.sha1(audio_bytes).hexdigest()
            assert by_name["Super Mario 64 (J) (Rev A) [Part 1].z64"].md5_hash == ""
            assert parsed.md5_hash == ""
        finally:
            shutil.rmtree(soundtrack_dir)

    @pytest.mark.asyncio
    async def test_get_rom_files_audio_hashed_on_non_hashable_platform(
        self, handler: FSRomsHandler, config
    ):
        """NON_HASHABLE_PLATFORMS skip rom hashing but not soundtrack files."""
        import hashlib

        win = Platform(name="Windows", slug="win", fs_slug="win")
        rom = Rom(
            id=9,
            fs_name="MyGame",
            fs_path="win/roms",
        ...
        audio_bytes = b"town-theme-bytes"
        rom_dir = handler.base_path / "win/roms/MyGame"
        (rom_dir / "soundtrack").mkdir(parents=True, exist_ok=True)
        (rom_dir / "game.exe").write_bytes(b"binary")
        (rom_dir / "soundtrack/town.mp3").write_bytes(audio_bytes)
        ...
        try:
            ...
            by_name = {rf.file_name: rf for rf in parsed.rom_files}
            assert by_name["game.exe"].md5_hash == ""
            audio = by_name["town.mp3"]
            assert audio.md5_hash == hashlib.md5(audio_bytes).hexdigest()
            assert audio.category == RomFileCategory.SOUNDTRACK
            assert parsed.md5_hash == ""
        finally:
            shutil.rmtree(handler.base_path / "win")
