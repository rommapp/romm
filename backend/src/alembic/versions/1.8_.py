...
def upgrade() -> None:
    ...
    if os.getenv('ROMM_DB_DRIVER') == 'mariadb':
        op.alter_column(table_name='platforms', column_name='slug', new_column_name='fs_slug', type_=sa.String(length=50), existing_type=sa.String(length=50))
        op.add_column(table_name='platforms', column=sa.Column('slug', sa.String(length=50)))
        op.execute("INSERT INTO platforms(slug) SELECT fs_slug from platforms")          
    with op.batch_alter_table("platforms") as batch_op:
        if os.getenv('ROMM_DB_DRIVER') == 'sqlite' or not os.getenv('ROMM_DB_DRIVER'):
