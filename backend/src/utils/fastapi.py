...
def scan_rom(platform: Platform, rom_attrs: dict, r_igbd_id_search: str = '', overwrite: bool = False) -> Rom:
    ...
    files: list = fs.get_rom_files(rom_attrs['multi'], rom_attrs['file_name'], roms_path)
    file_size, file_size_units = fs.get_file_size(rom_attrs['multi'], rom_attrs['file_name'], files, roms_path)
    ...
    rom_attrs.update({'file_path': roms_path, 'file_name': rom_attrs['file_name'], 'file_extension': get_file_extension(rom_attrs),
                      'multi': rom_attrs['multi'], 'files': files, 'file_size': file_size, 'file_size_units': file_size_units,
