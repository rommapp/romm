"""empty message

Revision ID: 0013_upgrade_file_extension
Revises: 0012_add_regions_languages
Create Date: 2024-01-02 16:51:20.485746

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import mysql

# revision identifiers, used by Alembic.
revision = "0013_upgrade_file_extension"
down_revision = "0012_add_regions_languages"
branch_labels = None
depends_on = None


def upgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("roms", schema=None) as batch_op:
        batch_op.alter_column(
            "file_extension", type_=sa.String(length=100), existing_type=sa.String(length=10)
        )

    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("roms", schema=None) as batch_op:
        batch_op.alter_column(
            "file_extension", type_=sa.String(length=10), existing_type=sa.String(length=100)
        )

    # ### end Alembic commands ###
