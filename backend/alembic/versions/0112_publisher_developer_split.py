...
Revision ID: 0112_publisher_developer_split
Revises: 0111_physical_roms
...
down_revision = "0111_physical_roms"
branch_labels = None
depends_on = None
...
# ---------------------------------------------------------------------------
# roms_facets mirror (mirrors 0100 + 0103; publishers/developers added)
# ---------------------------------------------------------------------------
...
# Appended by 0103; the source column on `roms` carries the same name.
_FACETS_PROVIDER_IDS = [
    (name, name)
    for name in (
        "igdb_id",
        "ss_id",
