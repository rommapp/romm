...
<template>
    ...
    <template v-if="platforms.value.length > 0">
        ...
        <!-- Platforms cards -->
        <v-row class="pa-2" no-gutters>
            <v-col v-for="platform in platforms.value" class="pa-1" :key="platform.slug" :cols="views[0]['size-cols']"
                :xs="views[0]['size-xs']" :sm="views[0]['size-sm']" :md="views[0]['size-md']" :lg="views[0]['size-lg']">
