...
async function massRename() {
  ...
  socket.on("mass_rename:done", () => {
    ...
    emitter.emit("refreshGallery");
    emitter.emit("snackbarShow", {
      msg: "Mass renaming successful!",
      icon: "mdi-check-bold",
      color: "green",
    });
    socket.close();
  });
  socket.emit("mass_rename", route.params.platform);
}
...
async function fetchRoms(platform) {
  ...
  await axios
    .get("/api/platforms/" + platform + "/roms")
    .then((response) => {
...
<template>
  ...
  <v-row v-show="currentSection == 'roms'" no-gutters>
    <v-row
      v-show="!gettingRoms && galleryView.value != 2 && roms.length > 0"
      class="pa-1"
      no-gutters
    >
      <v-col
        v-for="rom in romsFiltered"
        class="pa-1"
        :key="rom.file_name"
    ...
    <v-row v-if="!gettingRoms && roms.length == 0" no-gutters>
      <div class="text-h6 mt-16 mx-auto">
