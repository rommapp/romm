...
async function handleStop(): Promise<void> {
  ...
  // "exited" tells onBeforeUnmount the session is already released, so the
  // navigation below doesn't trigger a second DELETE.
...
async function handleSaveAndExit(): Promise<void> {
  ...
  if (!saved) {
    emitter?.emit("snackbarShow", {
      msg: "The emulator did not confirm the save - recent progress may be lost.",
      icon: "mdi-alert",
      color: "orange",
      timeout: 6000,
    });
