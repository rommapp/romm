...
<template>
    ...
    <div :class="{ 'content': lgAndUp, 'content-tablet': mdAndDown, 'content-mobile': xs }" v-if="rom !== undefined">
        <v-row class="pt-8 justify-center">
            <v-col :class="{ 'cover': lgAndUp, 'cover-tablet': mdAndDown, 'cover-mobile': xs }">
                <v-row>
                    <v-col>
                        <v-card elevation="2" :loading="downloadStore.value.includes(rom.file_name) ? 'rommAccent1' : null">
                            <v-img :src="`/assets/romm/resources/${rom.path_cover_l}`"
                                :lazy-src="`/assets/romm/resources/${rom.path_cover_s}`" cover>
                ...
                <v-row class="pl-3 pr-3 action-buttons">
                    <v-col class="pa-0">
                        <template v-if="rom.multi">
                            <v-btn @click="downloadRomApi(rom, filesToDownload)"
                                :disabled="downloadStore.value.includes(rom.file_name)" rounded="0" color="primary" block>
                    ...
                    <v-col class="pa-0">
                        <v-btn rounded="0" block :disabled="!saveFiles"><v-icon icon="mdi-content-save-all"
                                size="large" /></v-btn>
                    </v-col>
                    <v-col class="pa-0">
                        <v-menu location="bottom">
                            ...
                            <v-list rounded="0" class="pa-0">
                                <v-list-item @click="searchRomIGDB()" class="pt-4 pb-4 pr-5">
                                    <v-list-item-title class="d-flex"><v-icon icon="mdi-search-web" class="mr-2" />Search
                                        IGDB</v-list-item-title>
                                </v-list-item>
                                <v-divider class="border-opacity-25" />
                                <v-list-item @click="dialogEditRom = true" class="pt-4 pb-4 pr-5">
                                    <v-list-item-title class="d-flex"><v-icon icon="mdi-pencil-box"
                                            class="mr-2" />Edit</v-list-item-title>
                                </v-list-item>
                                <v-divider class="border-opacity-25" />
                                <v-list-item @click="dialogDeleteRom = true" class="pt-4 pb-4 pr-5 text-red">
                                    <v-list-item-title class="d-flex"><v-icon icon="mdi-delete"
            ...
            <v-col class="mt-10" :class="{ 'info': lgAndUp, 'info-tablet': mdAndDown, 'info-mobile': xs }">
                ...
                <div
                    ...
                    <v-tabs v-model="tab" slider-color="rommAccent1" rounded="0">
                        ...
                        <v-tab value="saves" rounded="0" disabled>Saves<span class="text-caption text-truncate ml-1">[coming
                                soon]</span></v-tab>
                    ...
                    <v-window v-model="tab" class="mt-2">
                        <v-window-item value="details">
                            <v-row v-if="!rom.multi" class="d-flex align-center text-body-1 mt-0">
                                <v-col cols="3" xs="3" sm="2" md="2" lg="2"
                                    class="font-weight-medium"><span>File</span></v-col>
                                <v-col class="text-body-1"><span>{{ rom.file_name }}</span></v-col>
                            </v-row>
                            <v-row v-if="rom.multi" class="d-flex align-center text-body-1 mt-0">
                                <v-col cols="3" xs="3" sm="2" md="2" lg="2"
                                    class="font-weight-medium"><span>Files</span></v-col>
                                <v-col><v-select :label="rom.file_name" item-title="file_name" v-model="filesToDownload"
                                        :items="rom.files" class="mt-2 mb-2" density="compact" variant="outlined"
                                        return-object multiple hide-details clearable chips /></v-col>
                            </v-row>
                            <v-row class="d-flex align-center text-body-1 mt-0">
                                <v-col cols="3" xs="3" sm="2" md="2" lg="2"
                                    class="font-weight-medium"><span>Size</span></v-col>
                            ...
                            <v-row v-if="rom.r_igdb_id != ''" class="d-flex align-center text-body-1 mt-0">
                                <v-col cols="3" xs="3" sm="2" md="2" lg="2"
                                    class="font-weight-medium"><span>IGDB</span></v-col>
                                <v-col>
                                    <v-chip variant="outlined" :href="`https://www.igdb.com/games/${rom.r_slug}`" label>{{
                                        rom.r_igdb_id }}</v-chip>
                                </v-col>
                            </v-row>
                            <v-row v-if="rom.tags.length > 0" class="d-flex align-center text-body-1 mt-0">
                                <v-col cols="3" xs="3" sm="2" md="2" lg="2"
                                    class="font-weight-medium"><span>Tags</span></v-col>
                                <v-col><v-chip-group class="pt-0"><v-chip v-for="tag in rom.tags" :key="tag" class="bg-chip"
                                            label>{{ tag }}</v-chip></v-chip-group></v-col>
                            </v-row>
                        ...
                        <v-window-item value="screenshots">
                            <v-row class="d-flex mt-2">
                                <v-carousel hide-delimiter-background delimiter-icon="mdi-square" class="bg-rommBlack"
                                    show-arrows="hover" height="400">
                                    <v-carousel-item v-for="screenshot in rom.path_screenshots"
    ...
    <v-dialog v-model="dialogSearchRom" scroll-strategy="none" width="auto" :scrim="false" v-if="rom !== undefined">
        <v-card :class="{ 'search-content': lgAndUp, 'search-content-tablet': mdAndDown, 'search-content-mobile': xs }"
            rounded="0">
            <v-toolbar density="compact" class="bg-primary">
                <v-row class="align-center" no-gutters>
                    ...
                    <v-col>
                        <v-btn @click="dialogSearchRom = false" class="bg-primary" rounded="0" variant="text"
                            icon="mdi-close" block />
                    </v-col>
                </v-row>
            </v-toolbar>
            ...
            <v-toolbar density="compact" class="bg-primary">
                <v-row class="align-center" no-gutters>
                    <v-col cols="7" xs="7" sm="8" md="8" lg="9">
                        <v-text-field @keyup.enter="searchRomIGDB()" @click:clear="searchTerm = ''" v-model="searchTerm"
                            label="search" hide-details clearable />
                    </v-col>
                    <v-col cols="3" xs="3" sm="2" md="2" lg="2">
                        <v-select label="by" :items="['ID', 'Name']" v-model="searchBy" hide-details />
                    </v-col>
                    <v-col cols="2" xs="2" sm="2" md="2" lg="1">
                        <v-btn type="submit" @click="searchRomIGDB()" class="bg-primary" rounded="0" variant="text"
                            icon="mdi-search-web" block />
                    </v-col>
                </v-row>
            </v-toolbar>
            ...
            <v-card-text class="pa-1 scroll bg-secondary">
                ...
                <v-row no-gutters>
                    <v-col class="pa-1" cols="4" xs="4" sm="3" md="3" lg="2" v-show="!searching" v-for="rom in matchedRoms"
                        :key="rom.file_name">
                        <v-hover v-slot="{ isHovering, props }">
                            <v-card @click="updateRom(updatedData = rom)" v-bind="props" :class="{ 'on-hover': isHovering }"
    ...
    <v-dialog v-model="dialogEditRom" scroll-strategy="none" width="auto" :scrim="false" v-if="rom !== undefined">
        <v-card rounded="0"
            :class="{ 'edit-content': lgAndUp, 'edit-content-tablet': mdAndDown, 'edit-content-mobile': xs }">
            <v-toolbar density="compact" class="bg-primary">
                <v-row class="align-center" no-gutters>
                    <v-col cols="9" xs="9" sm="10" md="10" lg="11">
                    ...
                    <v-col>
                        <v-btn @click="dialogEditRom = false" class="bg-primary" rounded="0" variant="text" icon="mdi-close"
                            block />
                    </v-col>
                </v-row>
            </v-toolbar>
            ...
            <v-card-text class="bg-secondary scroll">
                <v-row class="justify-center pa-2" no-gutters>
                    <v-text-field @keyup.enter="updateRom()" v-model="updatedRom.r_name" label="Name" variant="outlined"
                        required hide-details />
                </v-row>
                <v-row class="justify-center pa-2" no-gutters>
                    <v-text-field @keyup.enter="updateRom()" v-model="updatedRom.file_name" label="File name"
                        variant="outlined" required hide-details />
                </v-row>
                <v-row class="justify-center pa-2" no-gutters>
                    <v-textarea @keyup.enter="updateRom()" v-model="updatedRom.summary" label="Summary" variant="outlined"
                        required hide-details />
                </v-row>
                <v-row class="justify-center pa-2" no-gutters>
                    <v-file-input @keyup.enter="updateRom()" label="Custom cover [Coming soon]"
    ...
    <v-dialog v-model="dialogDeleteRom" width="auto" v-if="rom !== undefined">
        <v-card rounded="0"
            :class="{ 'delete-content': lgAndUp, 'delete-content-tablet': mdAndDown, 'delete-content-mobile': xs }">
            <v-toolbar density="compact" class="bg-primary">
                <v-row class="align-center" no-gutters>
                    <v-col cols="9" xs="9" sm="10" md="10" lg="11">
                    ...
                    <v-col>
                        <v-btn @click="dialogDeleteRom = false" class="bg-primary" rounded="0" variant="text"
                            icon="mdi-close" block />
                    </v-col>
                </v-row>
            </v-toolbar>
