        ...
        <v-row class="pt-8 justify-center">
            <v-col :class="{ 'cover': lgAndUp, 'cover-tablet': mdAndDown, 'cover-mobile': xs }">
                <v-row>
                    <v-col>
                        <v-card elevation="2" :loading="downloading.value.includes(rom.file_name) ? 'rommAccent1' : null">
                            <v-img :src="`/assets/romm/resources/${rom.path_cover_l}?reload=${Date.now()}`"
                                :lazy-src="`/assets/romm/resources/${rom.path_cover_s}?reload=${Date.now()}`" cover>
                ...
                <v-row class="pl-3 pr-3 action-buttons">
                    <v-col class="pa-0">
                        <v-btn @click="downloadRom(rom, emitter, filesToDownload)" rounded="0" color="primary" block><v-icon
                                icon="mdi-download" size="large" /></v-btn>
                    </v-col>
                    <v-col class="pa-0">
                        <v-btn @click="downloadSave(rom, emitter)" rounded="0" block :disabled="!saveFiles"><v-icon
                                icon="mdi-content-save-all" size="large" /></v-btn>
                    </v-col>
                    <v-col class="pa-0">
