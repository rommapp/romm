...
function stateDisabledReason(asset: Asset) {
  const emulator = "emulator" in asset ? asset.emulator : null;
  if (!emulator || emulator === window.EJS_core) return null;
  return t("play.state-incompatible-core", { emulator });
}
