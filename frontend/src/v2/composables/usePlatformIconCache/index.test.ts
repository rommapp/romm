...
describe("prefetchPlatformIcons", () => {
  it("fetches the shipped url for a known slug", async () => {
  ...
  it("does not fetch an unshipped slug", async () => {
    respondWith({});
    const { prefetchPlatformIcons, getCachedPlatformIcon } = await load();
  ...
  it("caches nothing when the shipped file is not an image", async () => {
  ...
  it("caches nothing when the shipped file fails in transport", async () => {
