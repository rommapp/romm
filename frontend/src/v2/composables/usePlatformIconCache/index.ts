...
import { shippedPlatformIconUrl } from "./iconCache";
...
/** Blob URL for a shipped icon, or null when it is missing or unreadable. */
async function fetchCandidateUrl(url: string): Promise<string | null> {
  try {
...
async function fetchOne(slug: string): Promise<void> {
  ...
  inflight.add(key);
  try {
    const blobUrl = await fetchCandidateUrl(url);
    if (blobUrl) {
...
/**
 * Drop a slug's cached entry. CachedPlatformIcon calls this when the
 * blob fails to decode so the next render can use the shipped URL.
 */
