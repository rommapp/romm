...
<template>
    <v-row no-gutters>
        <v-col>
            <v-list-item :to="`/platform/${$route.params.platform}/${rom.id}`" :value="rom.id" :key="rom.id">
                ...
                <template v-slot:prepend>
                    <v-avatar :rounded="0">
                        <v-progress-linear color="rommAccent1" :active="downloading.value.includes(rom.file_name)"
                            :indeterminate="true" absolute />
                        <v-img :src="`/assets/romm/resources/${rom.path_cover_s}?reload=${forceImgReload}`"
        ...
        <v-col cols="3" xs="3" sm="1" md="1" lg="1" class="d-flex justify-center align-center mr-4">
            <v-btn @click="downloadRom(rom, emitter)" icon="mdi-download" size="x-small" variant="text" />
            <v-btn @click="downloadSave(rom, emitter)" icon="mdi-content-save-all" size="x-small" variant="text"
                :disabled="!saveFiles" />
