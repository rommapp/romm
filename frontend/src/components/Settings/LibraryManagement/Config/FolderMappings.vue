...
const mappings = computed<Row[]>(() => {
    ...
    for (const folder of folders) {
        if (bindings[folder]) {
            ...
            const platform = supportedPlatforms.value.find(
                (p) => p.slug === slug,
            );
            const displayName =
                platform?.display_name || platform?.name || slug;
        ...
        if (auto) {
            ...
            const displayName =
                platform?.display_name || platform?.name || auto;
            result.push({
                fsSlug: folder,
                slug: auto,
                displayName,
...
async function updateMappingType(
    ...
    try {
        ...
        const currentType = mappings.value.find(
            (r) => r.fsSlug === fsSlug,
        )?.type;
        if (currentType === "alias") {
            await configApi.deletePlatformBindConfig({ fsSlug });
                ...
                <div class="d-flex align-center flex-nowrap justify-end">
                    <v-tooltip bottom max-width="500">
                        ...
                        <div>
                            <p class="mb-2">
                                <strong
                                    >{{ t("settings.folder-alias") }}:</strong
                                >
                                {{
                                    t(
                                        "settings.folder-mappings-tooltip-aliases",
                                    )
                                }}
                            </p>
                            <p>
                                ...
                                {{
                                    t(
                                        "settings.folder-mappings-tooltip-variants",
                                    )
                                }}
                            </p>
                            <p class="text-caption mt-2">
                                {{
                                    t(
                                        "settings.folder-mappings-mutually-exclusive",
                                    )
                                }}
                            </p>
                        </div>
                    ...
                    <template #activator="{ props }">
                        <div class="d-flex align-center justify-center">
                            <v-chip
                                ...
                                v-bind="props"
                                :color="
                                    item.type === 'alias' ? 'primary' : 'accent'
                                "
                                size="small"
                                label
