...
const theme = computed(() => {
  ...
  const def = PLATFORM_THEME_BY_SLUG[s.slug] || PLATFORM_THEME_BY_SLUG[s.fs_slug];
  if (def) {
    return {
      name: def.label,
  ...
  return { name: s.name, shortName: s.name, image: undefined, background: 'linear-gradient(135deg, #2b3242 0%, #1b2233 100%)', accent: '#f8b400' };
});
